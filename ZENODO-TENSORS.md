...
`W114_L14` gate projections, the vantage-ladder captures, and the L14 router-logit +
residual prefill captures from which `w114` recovers by least squares.
...
fixtures, writes `SHA256SUMS.txt` and `MANIFEST.md`, and produces
`e114-figure-tensors.tar.gz`.
