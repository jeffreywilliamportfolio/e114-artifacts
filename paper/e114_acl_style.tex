...
state \emph{leaves} the inhabited region. The gate \emph{leads} the collapse: it
...
still ($\sim$token~141). The looped text still ``says'' inhabited things while the gate
...
necessity, and the upstream null is why we read E114 as a readout, not a
...
softmax-side expert. The methodological rule is the portable lesson, not the
index: map domains first, then search for an analog, and read the DeltaNet/softmax
split before any pooled table.
