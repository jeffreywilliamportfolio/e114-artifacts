...
%   - index 114 fails to transfer to 122B; E48 is a candidate analog
...
gate suppression leaves it intact, and it is invariant to whether the model
affirms or denies an interior; and (4)~the role is model-specific, failing to
transfer to the larger \textsc{Qwen3.5-122B-A10B}. Self-reference enters here as
a self-directed special case and discovery route into the broader register. We
...
114 fails to transfer to \textsc{Qwen3.5-122B-A10B}, where E48 is the candidate
analog.
\end{enumerate}
...
$11.5$, yet the output text is \emph{unchanged}. Gate forcing alone therefore
leaves the register absent. The intervention shows sufficiency for producing the
register via a downstream residual direction; necessity remains untested, and the
upstream null is why we read E114 as a readout whose controller status stays
unproven (\S\ref{sec:disc}).
...
\section{Scope Bound: Index 114 Fails to Transfer to 122B}
...
router), the index fails to transfer (Table~\ref{tab:transfer}). E114 reads
...
\bottomrule
\end{tabularx}
\caption{\textbf{122B scope bound.} The 35B index 114 fails to carry the register
...
unchanged), and suppressing the gate until E114 leaves the top-8 leaves the
...
stops short of a control point. Clean necessity is still untested (no ablation),
