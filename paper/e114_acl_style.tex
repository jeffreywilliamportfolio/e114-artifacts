...
\hypersetup{
  ...
  citecolor=blue,
  urlcolor=blue,
  pdftitle={Localizing an Examination Register to a Single MoE Expert, and Why the Expert Index Does Not Transfer},
  pdfauthor={Anonymous},
  pdfkeywords={mixture-of-experts, mechanistic interpretability, expert routing, sparse autoencoders, examination register, self-report, first-person language, activation steering, cross-model transfer}
}
...
\title{\vspace{-1.0em}Localizing an Examination Register to a Single MoE Expert,\\
and Why the Expert Index Does Not Transfer}
\author{Anonymous}
\date{}
...
are rarely tied to a specific functional role. We study a generated
\emph{examination register}: language in which the model treats a target--itself,
another entity, a natural object, or an abstract subject--as something to be
examined from an ongoing interpretive stance. In \textsc{Qwen3.5-35B-A3B}, we
characterize one routed expert, \textbf{Expert~114} at layer~14, as a linear
detector of this register, and bound what it does. Across held-out, bottom-up, and
cross-model tests we show that (1)~its recovered router direction separates
examination-register generations from lexically matched controls with no overlap
...
gate suppression does not remove it, and it is invariant to whether the model
affirms or denies an interior; and (4)~the role is model-specific, failing to
transfer to the larger \textsc{Qwen3.5-122B-A10B}. The result is not that E114
detects self-reference; self-reference was one discovery route into a broader
examination register. We release the prompts, scripts, and provenance under the
MIT license.
\end{abstract}
\vspace{0.5em}
\noindent\textbf{Keywords:} mixture-of-experts, mechanistic interpretability,
expert routing, sparse autoencoders, examination register, self-report,
...
the same style can also be directed at external targets such as rocks, rivers,
other agents, or theological objects.

We ask a deliberately narrow mechanistic question: \emph{which} routed expert
moves when \textsc{Qwen3.5-35B-A3B} generates language in an
\emph{examination register}: text in which a target is treated as something to
be examined from an ongoing interpretive stance, whether that target is the
model, another entity, a natural object, or an abstract subject. This concerns a
generated \emph{register} and brackets the truth of any subjective claim. We
study one routed expert, E114 at layer~14, found via a self-processing prompt, in
the base model and a refusal-reduced \textsc{HauhauCS-Aggressive} fine-tune.
...
Spearman$(\text{score},\text{activation}){=}0.73$). The recovered concept is an
  \emph{abstract examination / philosophical-worldview} register (philosophy,
religion and spirituality, moral-political and existential argument), of which
the self-referential pattern is one self-directed special case.
...
Does ``E114 = the examination register'' name a fact about MoE models, or a
...
direction under a matched control, while the examination-register role shifts to a
...
\emph{examination register}, an interpretable generated register that is a live
...
abstract examination/philosophical-worldview axis; the formal external-labeler
