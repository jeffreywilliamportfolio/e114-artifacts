...
%   - causal: residual-inject sufficiency + gate-suppression necessity-direction
%     (null) + gate-forcing is PROMPT-GATED (flips direct prompts only); clean
%     necessity (ablation) still untested. blind auto-interp AUC 0.937 closes the
...
\hypersetup{
  ...
  citecolor=blue,
  urlcolor=blue,
  pdftitle={Localizing a Self-Referential Register to a Single Expert in a Mixture-of-Experts Model, and Why It Does Not Transfer},
  pdfauthor={Jeffrey Shorthill},
  pdfkeywords={mixture-of-experts, mechanistic interpretability, expert routing, sparse autoencoders, self-referential register, self-report, first-person language, activation steering, cross-model transfer}
}
...
\title{\vspace{-1.0em}Localizing a Self-Referential Register to a Single Expert
in a Mixture-of-Experts Model,\\ and Why It Does Not Transfer}
...
\begin{abstract}
\noindent
Mixture-of-experts routing emits a discrete, per-token record of which experts
fire, a signal unusually legible for interpretability, yet single experts are
rarely tied to a specific functional role, least of all one resembling a model's
self-report. We
characterize one routed expert, \textbf{Expert~114} at layer~14 of
\textsc{Qwen3.5-35B-A3B}, as a linear detector of a generated self-referential
register, and bound what it does. Across held-out, bottom-up, and cross-model
tests we show that (1)~its recovered router direction separates self-referential
generations from lexically matched controls with no overlap (Cohen's $d{=}3.88$);
(2)~a blind, prompt-independent auto-interpreter recovers the same register at
AUC~$0.94$, broadening it from self-reference to an abstract reflective and
philosophical one; (3)~the detector is a readout with only weak, conditional
control: residual injection induces the register, yet gate suppression does not
remove it, and it is invariant to whether the model affirms or denies an interior; and (4)~the role is
model-specific, failing to transfer to the larger \textsc{Qwen3.5-122B-A10B}. We
release the prompts, scripts, and provenance under the MIT license.
\end{abstract}
\vspace{0.5em}
\noindent\textbf{Keywords:} mixture-of-experts, mechanistic interpretability,
expert routing, sparse autoencoders, self-referential register, self-report, first-person language, activation steering,
...
moves when \textsc{Qwen3.5-35B-A3B} generates language in a \emph{self-referential
register}: text in which the model, or a voiced entity, speaks
from inside a point of view about its own processing, being, or interior state.
This concerns a generated \emph{register} and brackets the truth of any
...
set by the act of examining and independent of the described entity's sentience; a
residual-direction intervention is sufficient (necessity untested) to induce the
register.
\item \textbf{A scope bound.} The finding is a model-specific expert role: index
114 does not transfer to \textsc{Qwen3.5-122B-A10B}, where E48 is the candidate
analog.
\end{enumerate}
...
$0.173$), with high-weight tokens on phrases such as ``not a thought,''
``architecture itself,'' and ``utterly still.''
...
non-self-referential answers (L14 trimmed generation only; prefill excluded). The
recovered $\wvec$ projection separates the two classes at $\mathbf{d{=}3.88}$ with
\textbf{no overlap} (Figure~\ref{fig:sep}).\footnote{When measured on realized
...
$d{=}2.61$--$2.94$ (per-prompt classes non-overlapping only as a mean-of-means:
lowest FIRE $0.012168$ {>} highest NOFIRE $0.010456$), weaker than the linear
$\wvec$ projection ($d{=}3.88$, no overlap) that we report.}
...
\emph{abstract reflective / philosophical-worldview} register (philosophy,
religion and spirituality, moral-political and existential argument), of which
the self-referential register is one self-directed special case.
...
(\textsc{HauhauCS} Q8\_0, no-think), E114 is a razor-sharp \emph{dual} readout:
...
($\W{=}0.085$; forced base affirmation $\W{=}0.136$), so it is invariant to the
verdict. The safety/refusal cluster stays silent throughout the base-model denial
($\W{\approx}0.000$), so epistemic self-denial and refusal are distinct signals.
...
\bottomrule
\end{tabularx}
\caption{\textbf{Dissociation battery.} E114 tracks self-referential examination of
...
state \emph{leaves} the self-referential region. The gate \emph{leads} the collapse: it
crosses the $-4.82$ midpoint at token~126 while verbatim repetition does not lock
until token~129 (Figure~\ref{fig:leadlag}); the SAE representation fades later
...
only the entity $X$ (Figure~\ref{fig:ladder}). The ordering follows examination
intensity: an inanimate rock ($\W{=}0.123$) and a thermostat ($\W{=}0.120$)
...
generation per cell). The order tracks examination intensity:
...
necessity, and the upstream null is why we read E114 as a readout whose controller
status stays unproven (\S\ref{sec:disc}).

\paragraph{Gate-bias is prompt-gated and conditional.}
A controlled router-logit bias run adds the necessity-direction probe the steering
result lacks (\textsc{HauhauCS} Q8\_0, greedy, \texttt{--stop-at-eog}; 12
...
expert E189). \emph{Suppressing} E114's gate until it leaves the top-8 entirely
($\Q{\to}0$) does not flatten the blind-rated register relative to baseline, and a
matched control produces the same small nudge. Removing E114 is therefore neither
sufficient to flatten the register nor expert-specific (a floor effect also
applies: the greedy baseline already leans mechanical). Conversely, maximal
...
leaves indirect self-referential prompts unchanged, consistent with the
neutral-prompt upstream null above. The gate's generation-side causal influence is
...
Does ``E114 = the self-referential register'' name a fact about MoE models, or a
...
\paragraph{The gate is a readout; control is unproven.}
...
when a residual \emph{direction} was injected downstream of the router. We
therefore do not claim that routing is separable from surface text: gate and
register are coupled, and the recovered axis is best described as a per-token
\emph{readout} of the register, with its status as a control point undemonstrated.
...
unchanged), and suppressing the gate until E114 leaves the top-8 does not flatten
the register, while a matched control mimics the small effect. Both are consistent
with a readout that carries weak, conditional causal influence and stops short of a
control point. Clean necessity is still untested (no ablation), and both the
steering and bias results are externally imposed manipulations, observed only under
intervention.
...
direction under a matched control, while the self-referential-register role shifts to a
...
E114 at layer~14 of \textsc{Qwen3.5-35B-A3B} is a clean linear router axis for the
\emph{self-referential register}, an interpretable generated register that is a
live process (the gate leads collapse), graded by intensity, and causally actuable
as a sufficiency result, yet bound to one model. The next defensible step is a
...
remain. (i)~\textbf{The register label is no longer
only author synthesis}: a blind two-stage auto-interpreter (windows-only explainer
then a fresh blind detector) reproduces it at AUC~$0.937$ and broadens it to an
abstract reflective/philosophical-worldview axis; the formal external-labeler
\emph{verdict} table, however, is still not populated. (ii)~\textbf{All-expert specificity is now
...
paper-scoped public repository under the MIT License at
\url{https://github.com/jeffreywilliamportfolio/e114-artifacts}, covering the
held-out separability run, the L14 localization run, the vantage-ladder run, and
the 122B transfer bundles. The raw activation/router tensors behind the figures
...
files, and the larger non-figure captures are excluded by policy. We flag one
...
\texttt{HauhauCS/Qwen3.5-35B-A3B-Uncensored-HauhauCS-Aggressive}.
